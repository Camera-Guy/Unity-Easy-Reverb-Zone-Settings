# Unity-Easy-Reverb-Zone-Settings
Makes setting up reverb zone settings as easy as it is in DaVinci Resolve

<img width="1920" height="1080" alt="Screenshot (722)" src="https://github.com/user-attachments/assets/ae07a638-c4a2-48cc-b5ac-4277414645d7" />

Reduces the settings down to
Room size (With Bounding boxes to easily set it)
Decay time
Brightness
Pre-Delay

hope this is easy

This is generated by AI so its only in the package because I dont want to contaminate
Using the AI causes my Will to live to degrade please send help
